...
async function writeXlsxAndShare(rows: Record<string, unknown>[], filename: string): Promise<void> {
  ...
  return rows
    .map((r) => {
      ...
      return {
        ...
        category: (category === 'WHOLESALE' ? 'WHOLESALE' : 'RETAIL') as CustomerCategory,
...
export async function exportPayables(payables: PayableBySupplier[]): Promise<void> {
...
export async function exportPaymentTypes(paymentTypes: PaymentType[]): Promise<void> {
...
export async function importPaymentTypes(): Promise<CreatePaymentTypeDTO[] | null> {
  const rows = await pickAndReadXlsx();
  if (!rows) return null;
...
export async function exportProducts(products: Product[]): Promise<void> {
  const rows = products.map((p) => {
    const retailPrice = p.sellPrices?.find((sp: ProductPrice) => sp.type === 'RETAIL')?.price ?? 0;
    const retailMin =
      p.sellPrices?.find((sp: ProductPrice) => sp.type === 'RETAIL')?.minimumPurchase ?? 1;
    const wholesalePrice =
      p.sellPrices?.find((sp: ProductPrice) => sp.type === 'WHOLESALE')?.price ?? 0;
    const wholesaleMin =
      p.sellPrices?.find((sp: ProductPrice) => sp.type === 'WHOLESALE')?.minimumPurchase ?? 0;
...
export async function importProducts(
  ...
  for (const r of rows) {
    ...
    const type: 'DEFAULT' | 'MULTIUNIT' | 'VARIANTS' =
      typeRaw === 'MULTIUNIT' ? 'MULTIUNIT' : typeRaw === 'VARIANTS' ? 'VARIANTS' : 'DEFAULT';
...
export async function exportUsers(users: User[]): Promise<void> {
  const rows = users.map((u) => ({
    ...
    'Terakhir Login': u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleDateString('id-ID') : '-',
  }));
