...
import { CheckIcon, ChevronDown, RefreshCcw, SearchIcon } from 'lucide-react-native';
import { useState } from 'react';
import { HStack } from '../hstack';
import { Icon } from '../icon';
import { Input, InputField, InputIcon, InputSlot } from '../input';
import { Modal, ModalBackdrop, ModalBody, ModalContent, ModalHeader } from '../modal';
...
export default function SelectModal({
  ...
  return (
    <>
      ...
      >
        <Text className={!value ? 'text-background-400' : undefined}>{labelValue}</Text>
        <Icon as={ChevronDown} className={!value ? 'text-background-400' : undefined} />
      </Pressable>
      <Modal
        isOpen={open}
        onClose={() => {
          ...
          >
            <HStack space="sm" className="flex-1 items-center">
              <Input className="flex-1 border border-background-300 rounded-lg h-10">
                ...
                <InputField placeholder={searchPlaceholder} onChangeText={(v) => setSearch(v)} />
              </Input>
              <Pressable
                onPress={() => {
                  setOpen(false);
              ...
              {!!filteredOptions.length && !filteredOptionsGroup ? (
                filteredOptions.map((option) => (
                  ...
                  >
                    ...
                    {option.value === value && <Icon as={CheckIcon} className="text-primary-500" />}
                ...
                <Text className="text-lg text-typography-500 text-center p-10">Tidak ada data</Text>
              )}
            </VStack>
          </ModalBody>
        </ModalContent>
