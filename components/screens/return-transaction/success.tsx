...
export default function ReturnTransactionSuccess() {
  ...
  const totalAmount =
    returnData?.items?.reduce((acc, item) => acc + item.quantity * (item.sellPrice || 0), 0) ?? 0;
        ...
        <VStack className="items-center py-8">
          ...
          <Box className="w-full flex-row flex-wrap gap-y-4 p-4 border-b border-background-300">
            ...
            <HStack className="w-full flex-row justify-between">
              ...
              <Text className="font-bold text-lg">{returnData?.customerName}</Text>
            </HStack>
          </Box>
          <VStack space="md" className="w-full p-4">
            <Pressable
              ...
              onPress={() => {
                router.replace(`/(main)/management/return/transaction/receipt/${id}`);
              }}
            >
