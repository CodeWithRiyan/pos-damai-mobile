...
export default function PurchasingHistory({
  ...
  const [purchasingFilter, setPurchasingFilter] =
    useState<PurchasingFilterFormValues>(purchasingFilterInitialValues);
  ...
  const chartData = groupingChartData.map((d) => {
    ...
    return {
      value:
        chartCategory === "totalTransaction" ? d.totalTransaction : d.expenses,
      label: isToday
        ? parsed.format("HH")
        : isThisYear
    ...
    <VStack className="flex-1 bg-white">
      ...
      >
        <VStack space="md" className="p-4 items-center">
          <FilterAccordion
            title={
              isReport ? "Filter Laporan Pembelian" : "Filter Riwayat Pembelian"
            }
